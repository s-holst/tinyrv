...
class rvlinux(sim):
    ...
    def timer_fired(self):
        ...
        if current_time >= match_time: self.csr[self.csr_mip] |= 1<<7  # timer expired
        else: self.csr[self.csr_mip] &= ~(1<<7)
        if (self.csr[self.csr_mstatus]&0x8) and (self.csr[self.csr_mie]&self.csr[self.csr_mip]&(1<<7)):
